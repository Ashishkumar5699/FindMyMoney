...
export async function GET(req: NextRequest, { params }: { params: Promise<{ contactId: string }> }) {
  ...
  const upstream = await dotnetFetch(`/api/findmymoney/khata/${claims.nameid}/${contactId}/transactions`, { method: 'GET' }, extractBearer(req));
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ contactId: string }> }) {
  ...
  const upstream = await dotnetFetch(`/api/findmymoney/khata/${claims.nameid}/${contactId}/transactions`, { method: 'POST', body }, extractBearer(req));
  return NextResponse.json(await upstream.json(), { status: upstream.status });
}
